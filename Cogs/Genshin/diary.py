...
# 원석 수입 구성 원형차트
def create_pie_chart(categories: List[genshin.models.genshin.DiaryActionCategory], channel_id: int, user_id = int) -> None:
    labels = []
...
class Dirary(commands.Cog):
    ...
    async def diary(self, interaction: discord.Interaction):
        try:
            ...
            create_pie_chart(categories=data.categories, channel_id=interaction.channel_id, user_id=interaction.user.id)
